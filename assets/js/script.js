document.addEventListener('DOMContentLoaded', () => {
  ...
  const CONFIG_CONTATO = {
    ...
    instagram: "https://www.instagram.com/nextautomatik"
  };
  ...
  const initApplePreloader = () => {
    const preloader = document.getElementById('preloader');
    const applePath = preloader?.querySelector('svg path');
    const finishPreloader = () => {
      document.body.classList.remove('preloading', 'preloader-revealing');
      document.dispatchEvent(new CustomEvent('jessica:preloader-complete'));
    };

    if (!preloader || !applePath) {
      finishPreloader();
      return;
    }

    try {
      const pathLength = applePath.getTotalLength();
      applePath.style.setProperty('--apple-path-length', pathLength);
    } catch (error) {
      applePath.style.setProperty('--apple-path-length', 11580);
    }
...
// ==========================================
  ...
  if (typeof gsap !== 'undefined') {
    ...
    if (document.body.classList.contains('preloading')) {
      ...
      document.addEventListener('jessica:preloader-complete', () => {
...
// ==========================================
    ...
    if (document.querySelector('.especialidade-conteudo')) {
    ...
    if (document.querySelector('.especialidade-hero-content')) {
    ...
    if (document.querySelector('.outras-grid')) {
      gsap.to(".outra-card", {
        scrollTrigger: {
          trigger: ".outras-grid",
          start: "top 85%",
...
document.querySelectorAll('.footer-logo-link').forEach(link => {
    ...
    // Rolagem suave no carregamento se a URL tiver hash
    if (window.location.hash) {
      const hashTarget = document.querySelector(window.location.hash);
      if (hashTarget) {
        setTimeout(() => {
          if (smoother) {
            smoother.scrollTo(hashTarget, true, "top 80px");
          } else {
            hashTarget.scrollIntoView({ behavior: 'smooth' });
          }
        }, 400); // Atraso para garantir inicialização completa do layout e imagens
      }
    }
  } else {
